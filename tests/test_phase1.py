...
def test_models_endpoint_rejects_missing_and_invalid_tokens(tmp_path):
...
def test_models_endpoint_returns_clean_error_when_ollama_is_unavailable(tmp_path, monkeypatch):
    settings = Settings(database_path=tmp_path / "proxy.sqlite3")
    initialize_database(settings.database_path)
    seed_dev_data(settings.database_path)
    app = create_app(settings)
